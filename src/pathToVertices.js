...
function pathToVertices(path, sampleLength) {
  if (typeof window !== 'undefined' && !('SVGPathSeg' in window)) {
    console.warn('Svg.pathToVertices: SVGPathSeg not defined, a polyfill is required.');
  }

  // https://github.com/wout/svg.topoly.js/blob/master/svg.topoly.js
  let i; let il; let total; let point; let segment; let segments;
  let segmentsQueue; let lastSegment;
  let lastPoint; let segmentIndex; const points = [];
  let lx; let ly; let length = 0;
  let x = 0;
  let y = 0;
 ...
 * @param {SVGPathElement} path
 */
function svgPathToAbsolute(path) {
  // http://phrogz.net/convert-svg-path-to-all-absolute-commands
  // Copyright (c) Gavin Kistner
  // http://phrogz.net/js/_ReuseLicense.txt
  // Modifications: tidy formatting and naming
  let x0; let y0; let x1; let y1; let x2; let y2; const segs = path.pathSegList;
  let x = 0;
  let y = 0;
